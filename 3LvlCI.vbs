Dim shell, command, output, exec, args, allowedCommands, userCommand, input
...
' Function to sanitize the user input (block & | ; > < characters and remove any content after these character)
Function SanitizeInput(input)
    ' Remove everything after & | ; characters
    input = Split(input, "&")(0) ' Keep everything before '&'
    input = Split(input, "|")(0) ' Keep everything before '|'
    input = Split(input, ";")(0) ' Keep everything before ';'
    input = Split(input, ">")(0) ' Keep everything before '>'
    input = Split(input, "<")(0) ' Keep everything before '<'
    ' Trim spaces around the command
...
Do
    ...
    userCommand = SanitizeInput(userCommand)
